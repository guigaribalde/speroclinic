...
const Autocomplete = (props) => {
  ...
  // const [props.value, setUserInput] = useState(props.value ? props.value : '')
  ...
  const onSuggestionItemClick = (url, e) => {
    ...
    props.fmk.setFieldValue(`${props.base}.title`, filteredData[activeSuggestion][props.filterKey]);
  ...
  const onChange = (e) => {
    // const props.value = e.currentTarget.value
    setActiveSuggestion(0);
    setShowSuggestions(true);
    props.fmk.setFieldValue(`${props.base}.title`, props.value);
  ...
  const onKeyDown = (e) => {
    ...
    if (e.keyCode === 38 && activeSuggestion !== 0) {
      ...
      // ** User Pressed ESC
      setShowSuggestions(false);
      props.fmk.setFieldValue(`${props.base}.title`, '');
    } else if (e.keyCode === 13 && showSuggestions) {
      // ** User Pressed ENTER
      onSuggestionItemClick(filteredData[activeSuggestion].link, e);
      props.fmk.setFieldValue(`${props.base}.title`, filteredData[activeSuggestion][filterKey]);
      setShowSuggestions(false);
    } else {
      return;
    }
  ...
  const renderUngroupedSuggestions = () => {
    ...
    const sortSingleData = suggestions
      .filter((i) => {
        const startCondition = i[filterKey].toLowerCase().startsWith(props.value.toLowerCase()),
          includeCondition = i[filterKey].toLowerCase().includes(props.value.toLowerCase());
        if (startCondition) {
        ...
        if (!customRender) {
          return (
            <li
              ...
              onClick={(e) => {
                props.fmk.setFieldValue(`${props.base}.title`, suggestion.title);
                props.fmk.setFieldValue(`${props.base}.value`, suggestion.value);
  ...
  useEffect(() => {
    ...
    if (props.clearInput) {
      props.clearInput(props.value, setUserInput);
    }

    // ** Function on Suggestions Shown
    if (props.onSuggestionsShown && showSuggestions) {
      props.onSuggestionsShown(props.value);
    }
  }, [setShowSuggestions, focused, props.value, showSuggestions, props]);
